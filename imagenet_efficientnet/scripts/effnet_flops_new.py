"""
Compute number of multiply and add operations *separately*.

If there is quantization performed anywhere, we should count
those operations as fractions of 32.
...
TODO:
* check the remaining implementations (linear and avg pooling)
...
def ops_conv(x, layer, is_not_quantized=False):
    # !!! TODO: check whether or not output channels need to be counted here.
    ...
    out_c = layer.weight.size(0)  # (cout, kernel_h, kernel_w, cin)

    kernel_els, full_kernel_els = (layer.weight != 0).float().sum(), layer.weight.numel()
    output_els = out_h * out_w  # * out_c
...
def ops_non_linearity(x, is_not_quantized):
    bits_ratio = 1.0 if is_not_quantized else 4.0 / BITS_BASE
    delta_ops = x.numel() / x.size(0) * bits_ratio
