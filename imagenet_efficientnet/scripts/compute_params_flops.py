...
BEST_CKPT_PATH = torch.load('../checkpoints/effnet_imagenet_prune_base2_best.pth.tar')
...
def load_ckpt():
    ...
    state_dict = _normalize_distiller_state_dict(state_dict)
    model.load_state_dict(state_dict)
    return model


def _is_not_quantized(name):
    return 'bias' in name or any([name.startswith(prefix) for prefix in FULL_PRECISION])
